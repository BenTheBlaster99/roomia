...
export function buildKitchenComposePrompt(opts: {
  ...
  const roomLines = opts.imagine
    ? [
        'There is no source photo. Invent one ordinary apartment kitchen from the client choices only.',
        'Eye-level camera facing the cabinet run. One room, lived-in, no people, no extra rooms.',
        opts.plan && kitchenPlanSummary(opts.plan)
          ? `Size the room from these typed millimetres, without drawing them: ${kitchenPlanSummary(opts.plan)}`
          : null,
        'Do not draw dimension lines, millimetre text, or a floor-plan overlay.',
      ]
    : [
        ...
        opts.plan ? kitchenPlanPrompt(opts.plan) : null,
      ]
  return [
    ...roomLines,
    ...
    opts.imagine
      ? 'Put the sink and the hob on the run. Nothing is measured from a photo.'
...
export type KitchenOpening = 'none' | 'left' | 'center' | 'right'

export type KitchenPlan = {
  runMm: string
  heightMm: string
  window: KitchenOpening
  door: KitchenOpening
}

const OPENING_FR: Record<KitchenOpening, string> = {
  none: '',
  left: 'à gauche',
  center: 'au centre',
  right: 'à droite',
...
/** What they typed. Null when the plan block is empty. */
export function kitchenPlanSummary(plan: KitchenPlan): string | null {
  const bits: string[] = []
  const run = plan.runMm.trim()
  const height = plan.heightMm.trim()
  if (run) bits.push(`linéaire ${run} mm`)
  if (height) bits.push(`hauteur ${height} mm`)
  if (plan.window !== 'none') bits.push(`fenêtre ${OPENING_FR[plan.window]}`)
  if (plan.door !== 'none') bits.push(`porte ${OPENING_FR[plan.door]}`)
  return bits.length > 0 ? bits.join(' · ') : null
}

export function kitchenPlanPrompt(plan: KitchenPlan): string | null {
  const summary = kitchenPlanSummary(plan)
  if (!summary) return null
  return [
    `CLIENT PLAN (numbers they typed, not measured from the photo): ${summary}`,
    'Keep this room. Put the window and the door only on walls already visible, on the side they named.',
    'Do not draw dimension lines, millimetre overlays, or a new floor plan.',
  ].join('. ')
}
